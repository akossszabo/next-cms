...
class CommandsListController extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="flex flex-col rounded-lg shadow-lg border-1 border-warm-gray-200/70 bg-white p-1">
        {items.map(({ title, icon }, idx) => (
          ...
          >
            <div className="flex flex-row items-center p-1 w-50 hover:(bg-warm-gray-200/40) rounded-md cursor-pointer">
              <span className="ml-2 mr-4">{icon}</span>
              {title}{" "}
            </div>
          </div>
        ))}
